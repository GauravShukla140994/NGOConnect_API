-- ============================================================
-- NGOConnect Patch: Project_List — exclude suspended/inactive org projects
--                                  from public volunteer browse
-- Date:    2026-07-29
-- Problem:
--   Volunteers could see projects belonging to orgs that were suspended,
--   pending approval, rejected, or otherwise not in APPROVED status.
--   The SP filtered org IsDeleted=0 but never checked o.StatusLkpId.
--
-- Fix:
--   Add v_ApprovedOrgLkpId (resolved from ORG_STATUS / APPROVED lookup).
--   Both the main SELECT and the TotalCount SELECT now include:
--       AND (p_OrgId IS NOT NULL OR o.StatusLkpId = v_ApprovedOrgLkpId)
--   Admin browse (p_OrgId IS NOT NULL) is unaffected — admins can still
--   view their own org's projects regardless of org status.
--
