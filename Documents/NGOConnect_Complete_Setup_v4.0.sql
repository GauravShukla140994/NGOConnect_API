...
--
-- v4.0 vs v3.0 key changes:
--   NEW TABLES : OrgMembershipRequests, SchemaVersions, CommunityPostAcknowledgements
--   NEW COLUMN : OrgMembers.LocationSharingLkpId
--   NEW LOOKUP : LOCATION_SHARING (LookupType #44)
--   MODIFIED SPs (18): User_GetProfile, User_UpdateProfile, Org_Register,
--     Org_Update, Org_GetProfile, Org_GetMembers, Org_List,
--     Project_Create, Project_GetById, Project_Update, Project_List,
--     Post_GetFeed, Post_GetById, Community_CreatePost, Community_GetFeed,
--     Sos_GetActive, Sos_Resolve
--   NEW SPs (33): Org_RequestMembership, Org_ReviewMembership,
--     Org_GetPendingMembers, Org_UpdateMemberPermissions, Org_UploadDocument,
--     User_UpdateSafetyPrefs, User_SaveInterests, User_UploadDocument,
--     Project_AddSkill, Project_GetSkills, Project_Complete,
--     Post_Delete, Post_Pin, Post_Update,
--     Community_GetPostById, Community_DeletePost, Community_AcknowledgePost,
--     Sos_ApproveResponder, Sos_Cancel, Sos_GetLatestLocation, Sos_GetById,
--     Donation_PauseRecurring, Donation_ResumeRecurring, Donation_GetAnnualSummary,
--     Donation_GetSupportedNGOs, Donation_GetDonors, Donation_GetOrgTransactions,
--     Withdrawal_Create, Withdrawal_GetByOrg, Withdrawal_AdminReview,
--     Certificate_GetByUser, UserSkillRating_Add, UserBadge_Award,
--     Application_Cancel
-- ============================================================
...
BEGIN
    SELECT lv.LookupValueId, lv.ValueCode, lv.ValueName, lv.Description
    FROM LookupValues lv JOIN LookupTypes lt ON lv.LookupTypeId = lt.LookupTypeId
    WHERE lt.TypeCode = p_TypeCode AND lv.ValueCode = p_ValueCode AND lv.IsDeleted = 0 LIMIT 1;
END //
...
BEGIN
    SELECT
        u.UserId, u.Mobile, u.Email, u.IsVerified,
        up.FirstName, up.LastName, up.Bio AS Bio, up.ProfilePhoto,
        up.DateOfBirth, up.Occupation, up.Organisation,
...
BEGIN
    ...
    ON DUPLICATE KEY UPDATE
        EmergVisibilityLkpId = p_EmergVisibilityLkpId,
        AutoShareDurLkpId    = p_AutoShareDurLkpId,
        AllowLocDuringSos    = p_AllowLocDuringSos,
        AllowLocDuringProj   = p_AllowLocDuringProj,
        UpdatedAt            = NOW();
...
CREATE PROCEDURE User_SaveInterests(
    IN p_UserId    INT UNSIGNED,
    IN p_Interests TEXT
    -- p_Interests is comma-separated: 'Education,Environment,Healthcare'
)
BEGIN
    DELETE FROM UserInterests WHERE UserId = p_UserId;
    INSERT INTO UserInterests (UserId, InterestName)
    SELECT p_UserId, TRIM(j.Interest)
    FROM JSON_TABLE(
        CONCAT('["', REPLACE(p_Interests, ',', '","'), '"]'),
        '$[*]' COLUMNS (Interest VARCHAR(100) PATH '$')
    ) j
    WHERE TRIM(j.Interest) != '';
...
BEGIN
    ...
    ELSE
        ...
        SELECT LookupValueId INTO v_MemStatLkpId FROM LookupValues lv JOIN LookupTypes lt ON lv.LookupTypeId = lt.LookupTypeId
        WHERE lt.TypeCode = 'MEMBER_STATUS' AND lv.ValueCode = 'APPROVED' LIMIT 1;

        INSERT INTO Organisations
            (OrgName, OrgTypeLkpId, RegNumber, Category, About, Mission, Vision,
             LogoUrl, ContactEmail, ContactPhone, Website,
             AddressLine1, AddressLine2, City, State, Pincode, Country, StatusLkpId, CreatedBy)
        VALUES
            (p_OrgName, p_OrgTypeLkpId, p_RegistrationNo, p_Category, p_About, p_Mission, p_Vision,
             p_LogoUrl, p_ContactEmail, p_ContactPhone, p_Website,
        ...
        VALUES (v_OrgId, p_UserId, v_RoleLkpId, v_MemStatLkpId, 1, 1, 1, 50, NOW(), p_UserId);
...
BEGIN
    SELECT
        ...
        tv.ValueName AS OrgType, sv.ValueName AS OrgStatus,
        o.CreatedAt,
        (SELECT COUNT(*) FROM OrgMembers WHERE OrgId = o.OrgId AND IsDeleted = 0
            AND StatusLkpId = (SELECT LookupValueId FROM LookupValues lv JOIN LookupTypes lt ON lv.LookupTypeId = lt.LookupTypeId WHERE lt.TypeCode='MEMBER_STATUS' AND lv.ValueCode='APPROVED')) AS MemberCount
...
BEGIN
    ...
    IF p_StatusCode IS NOT NULL THEN
        SELECT LookupValueId INTO v_StatusLkpId FROM LookupValues lv JOIN LookupTypes lt ON lv.LookupTypeId = lt.LookupTypeId
        WHERE lt.TypeCode = 'ORG_STATUS' AND lv.ValueCode = p_StatusCode LIMIT 1;
    END IF;

    SELECT o.OrgId, o.OrgName, o.Category, o.LogoUrl, o.City, o.State, sv.ValueName AS Status,
    ...
    WHERE o.IsDeleted = 0
      ...
      AND (p_City       IS NULL OR o.City LIKE CONCAT('%', p_City, '%'))
      AND (v_StatusLkpId IS NULL OR o.StatusLkpId = v_StatusLkpId)
...
BEGIN
    ...
    IF p_MediaUrls IS NOT NULL AND p_MediaUrls != '' THEN
        SELECT LookupValueId INTO v_MediaTypeLkpId FROM LookupValues lv JOIN LookupTypes lt ON lv.LookupTypeId = lt.LookupTypeId
        WHERE lt.TypeCode = 'MEDIA_TYPE' AND lv.ValueCode = UPPER(COALESCE(p_MediaType, 'IMAGE')) LIMIT 1;

        INSERT INTO PostMedia (PostId, FileUrl, MediaTypeLkpId, SortOrder)
        SELECT v_PostId, TRIM(j.url), v_MediaTypeLkpId, ROW_NUMBER() OVER () AS SortOrder
        FROM JSON_TABLE(
...
-- v4.0 MODIFIED: joins PostMedia, supports OrgId filter
...
BEGIN
    ...
    SELECT
        ...
        p.LikeCount, p.CommentCount, p.IsPinned, p.CreatedAt,
    ...
    SELECT COUNT(*) AS TotalCount FROM Posts p
    WHERE p.IsDeleted = 0 AND (p_OrgId IS NULL OR p.OrgId = p_OrgId);
END //
...
BEGIN
    INSERT IGNORE INTO UserBadges (UserId, BadgeLkpId, AwardedBy, OrgId, ProjectId)
    VALUES (p_UserId, p_BadgeLkpId, p_AwardedBy, p_OrgId, p_ProjectId);
    SELECT 1 AS IsSuccess, 'Badge awarded.' AS Message, LAST_INSERT_ID() AS UserBadgeId;
END //
...
VALUES ('v4.0', 'Full platform setup: 45 tables, 44 LookupTypes, 80+ stored procedures. Added OrgMembershipRequests, CommunityPostAcknowledgements, SchemaVersions. Added LocationSharingLkpId to OrgMembers. Added CancelReason to SosIncidents, CanViewLocation to SosResponders, PausedAt to RecurringDonations. All 31 platform gaps resolved.', 'System');
...
UNION ALL
SELECT 'SCHEMA_VERSION', Version FROM SchemaVersions ORDER BY AppliedAt DESC LIMIT 1;

-- Expected: TABLES=47, STORED_PROCEDURES=100, LOOKUP_TYPES=44, LOOKUP_VALUES=160+, SETTINGS=21, SCHEMA_VERSION=v4.0

-- ── END OF NGOConnect_Complete_Setup_v4.0.sql ────────────────────
