# NGOConnect Database Documentation v4.8
...
| PROFILE_VERIFICATION_STATUS | **v4.6 NEW** — Not Reviewed (PENDING), Verified (VERIFIED), Needs Update (NEEDS_UPDATE) |
...
| Org_Resubmit | p_OrgId, p_UserId, p_OrgName, p_Category, p_ContactPerson, p_About, p_Mission, p_Vision, p_LogoUrl, p_ContactEmail, p_ContactPhone, p_Website, p_AddressLine1, p_AddressLine2, p_City, p_State, p_Pincode, p_Country | WRITE | **v4.5 NEW (founder-side)** 19 params. Updates org fields + resets status to PENDING. Guards: caller must be FOUNDER; org must be in REJECTED status. Inserts OrgStatusHistory row (ChangedByType=FOUNDER). Returns `IsSuccess`, `Message` |
...
| SP Name | Params | Type | Description |
|---|---|---|---|
| Community_CreatePost | p_UserId, p_OrgId, p_Title, p_Content, p_PostTypeLkpId, p_AudienceLkpId | WRITE | **v4.8 FIXED** Default audience lookup TypeCode corrected to `AUDIENCE_TYPE` (was wrong `POST_VISIBILITY`). Creates org-scoped community post. Auto-defaults `AudienceLkpId` to ALL_MEMBERS if NULL. Returns `IsSuccess`, `Message`, `CommunityPostId` |
...
| Community_CreatePoll | p_UserId, p_OrgId, p_Question, p_OptionsJson, p_ExpiresInHours, p_IsMultiChoice | WRITE | **v4.8 UPDATED** 6 params. New `p_IsMultiChoice TINYINT(1) DEFAULT 0` — persisted to `CommunityPosts.PollIsMultiChoice`. Default audience TypeCode fixed to `AUDIENCE_TYPE`. Creates CommunityPost of type POLL + PollOptions from JSON array. Returns `IsSuccess`, `Message`, `PollId` |
...
| SuperAdmin_Org_GetDetail | p_OrgId | READ | Full org detail: all list fields + About, Mission, Vision, FounderName, FounderEmail, FounderMobile, AddressLine, City, State, Pincode. **v4.6:** added `MemberCount` (APPROVED members count) |
...
| Org_Resubmit *(founder-side, counted under Organisation)* | p_OrgId, p_UserId + 17 org fields | WRITE | **State guard:** REJECTED only → PENDING. Caller must be FOUNDER. Updates org fields + inserts OrgStatusHistory (ChangedByType=FOUNDER) |
...
*Database_Documentation_v4.8.md — 53 Tables, 45 LookupTypes, 143 Stored Procedures — Last updated 2026-07-12*
