-- ============================================================
-- patch_fix_category_in_sps.sql
-- Adds CategoryName (p.Category AS CategoryName) to:
--   - User_GetImpactSummary (all 4 result sets)
--   - Application_GetByUser (full rebuild with paging + schedule fields)
...
BEGIN
    ...
    BEGIN
        ...
        SELECT ROUND(COALESCE(SUM(pa.HoursLogged), 0), 1) INTO v_TotalHours
        FROM   ProjectAttendance pa WHERE pa.UserId = p_UserId AND pa.AttendStatusLkpId = v_AttStatusAttended;
