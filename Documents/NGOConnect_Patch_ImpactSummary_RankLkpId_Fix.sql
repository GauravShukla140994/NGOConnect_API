-- ============================================================
-- NGOConnect_Patch_ImpactSummary_RankLkpId_Fix.sql
--
-- HOTFIX: Unknown column 'up.RankLkpId' in User_GetImpactSummary
--
-- Root cause: NGOConnect_Patch_CertificateIssuance.sql contained
-- a stale RS6 block that referenced up.RankLkpId and up.ReliabilityPct —
-- columns that do not exist on UserProfiles. This crashed every call
-- to GET /api/v1/user/impact-summary with a 200 status but empty data.
--
-- Fix: Replaces User_GetImpactSummary with the correct version that
-- computes ImpactScore / ReliabilityPct / RankName from scratch using
-- TIMESTAMPDIFF and a CASE expression. Identical to NGOConnect_Complete_Setup_v4.9.sql.
--
-- Apply to: Railway staging → Railway production
-- Safe to re-run: YES (DROP + CREATE is idempotent)
-- ============================================================
...
BEGIN
    ...
    -- RS6: Impact stats (computed from scratch — no stored columns on UserProfiles)
    BEGIN
...
SELECT 'HOTFIX applied: User_GetImpactSummary — removed up.RankLkpId reference (column does not exist on UserProfiles)' AS Result;
