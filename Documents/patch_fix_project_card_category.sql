-- ── patch_fix_project_card_category.sql ─────────────────────────────────────
-- Feature : Show project category and correct schedule type on all project card
--           screens (Impact tab Upcoming cards, My Projects screen all tabs).
--
-- Root cause:
--   1. User_GetImpactSummary — all 4 result sets (RS0 Applied, RS1 Upcoming,
--      RS2 Completed, RS3 Cancelled) omitted p.Category.
--   2. Application_GetByUser — also omitted p.Category.
--
-- Change (2 SPs):
--   User_GetImpactSummary : added p.Category AS CategoryName to all 4 SELECTs.
--   Application_GetByUser : added p.Category AS CategoryName to SELECT.
--
