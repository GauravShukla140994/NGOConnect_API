...
**Super Admin proactive Member + Organisation onboarding — Phase 4: deferred deep linking (2026-08-18, decision only)**
- User decision: skip for now. Deep links (`ripplehub.app/ngo/{token}`, etc.) already open the app fine if it's installed (existing `openApp()` custom-scheme attempt in `useDeepLinkLanding.js`, unchanged). What's explicitly NOT built: deferred deep linking — if the app isn't installed, the org/opportunity context is lost after the store install/open, and the person has to be re-sent the link. No Branch.io or custom pending-association solution built. Revisit if this friction turns out to matter in practice once Phase 1-3 are live and in use.

**Super Admin proactive Member + Organisation onboarding — Phase 3: rich public org profile (2026-08-18, same session continued)**
- User decision (2026-08-18): "mostly parameter in the organisation table can be used as public" — no separate per-field visibility flag system needed now. Applied as: expose essentially all `Organisations` columns except internal audit IDs (CreatedBy/UpdatedBy/DeletedBy/StatusUpdatedBy — raw IDs, meaningless publicly) and the plan-gate columns (CanCreateRecurring/CanCreateFlexible/OrgMaxVolunteers — internal business logic, not profile info); `OrgDonationSettings` limited to `IsDonationEnabled`/`Is80GEligible`/`Is12AEligible` only — banking/PAN/KYC/Razorpay fields never touched.
- Scope turned out much smaller than expected — reviews/ratings (`OrgReview_GetList`, `OrgReview_GetAggregate`) and project listing (`Project_List`) **already existed and needed zero SP changes**. Only the org info block itself needed a new SP.
- `NGOConnect_Complete_Setup_v5.0.sql`: new SP `Org_GetPublicProfile` — replaces the thin `Org_GetPublicPreview` for the /ngo/{token} page. Returns a `ProfileState` gate (`NOT_FOUND` / `UNAVAILABLE` / `ACTIVE`), mirroring the `CERT_REVOKED`/`NOT_FOUND` pattern `CertificateController` already uses — `UNAVAILABLE` covers a link shared before the org went PENDING/REJECTED/SUSPENDED (BRD Section 13 edge case), only OrgId/OrgName returned in that case.
- `NGOConnect.Core/Interfaces/IOrgDal.cs` + `NGOConnect.Infrastructure/DAL/OrgDal.cs`: new `GetPublicProfileAsync` — translates `ProfileState` into `NOT_FOUND`/`ORG_UNAVAILABLE` ErrorCodes.
- `NGOConnect.API/Controllers/PublicController.cs`: two new endpoints — `GET /api/v1/public/org/{token}/full` (profile + review aggregate + first page of projects, parallel-fetched) and `GET /api/v1/public/org/{token}/reviews` (paginated, `pageSize` clamped 1-50 since it's unauthenticated). Constructor now also takes `IOrgReviewDal` (already DI-registered, no config change needed).
- **Security finding caught during implementation**: `Project_List`'s built-in "public browse" whitelist (ACTIVE/UPCOMING only, `IsPublic=1` only) is skipped entirely whenever `p_OrgId` is supplied — that branch exists for an org's own admin viewing everything including DRAFT/CANCELLED/private projects. Since `GetOrgFullProfile` calls it anonymously with `orgId` set, it would otherwise leak non-public projects. Did **not** touch the shared SP (used by authenticated flows too, out of blast-radius) — filtered `ACTIVE`/`UPCOMING`/`COMPLETED` + `IsPublic=1` in the controller instead before returning to the client.
- `Documents/patch_org_public_profile.sql` (new) — SP-only, no table changes.
- `validate_sp_params.py` — all phases passed.
- `Website/src/hooks/useDeepLinkLanding.js`: added `ORG_UNAVAILABLE` to `friendlyError`.
- `Website/src/pages/NgoLandingPage.jsx`: switched from `/public/org/{token}` (thin) to `/public/org/{token}/full` (rich) — now shows About (3-line clamp), Verified badge, rating + review count, active/completed project counts, and up to 3 recent project names.
- **Deliberately NOT built** (flagged, not silently skipped): this page still auto-attempts the app deep link on load (same UX as every other `/invite`/`/opportunity` landing page) — it's a richer bounce-through card, not a standalone web page a visitor browses with paginated review scrolling (BRD Section 8's "pagination/infinite scrolling for large numbers of reviews" needs a page that does NOT auto-redirect). Building that stay-on-web page is separate remaining work if the bounce-to-app UX isn't what's wanted for organisations without the app installed.
- Both repos build-verified (API: not build-verified, no local .NET SDK in sandbox — verify before deploying; Website: 881 modules, passed).

**Super Admin proactive Member + Organisation onboarding — Phase 2: wizard UI (2026-08-18, same session continued)**
- `Website/src/admin/pages/CreateMemberPage.jsx` (new) — 4-step wizard (Member → Organisation → Role → Review) + a 5th success screen (Section 6: Copy Link / Share Link via `navigator.clipboard` / `navigator.share`, falls back to copy if Web Share API unavailable). Org step supports both "Associate existing" (dropdown, `getOrgsByStatus('APPROVED')`) and "Create new" (full org form) per BRD Option A/B. Role step maps to the 4 real `MEMBER_ROLE` values (FOUNDER/ADMIN/MODERATOR/MEMBER) — no separate "Volunteer" org role exists in the lookup data (see Phase 1 note); labelled "Member / Volunteer" to match user mental model without inventing a new lookup value.
- `Website/src/admin/api/members.js`: added `createMemberWithOrg(payload)` → `POST /superadmin/members`.
- `Website/src/admin/AdminApp.jsx`: added route `members/create` → `CreateMemberPage`.
- `Website/src/admin/pages/MembersPage.jsx`: added "+ Create member" button in the header, navigates to the new route.
- Client-side validation before each step advance (at least one of Email/Mobile; required NEW-org fields) — mirrors but does not replace the SP's own server-side validation, which remains the source of truth (duplicate email/mobile still hard-rejected server-side even if the client-side check is bypassed).
- Website build verified (881 modules).
- Not yet built: org document upload during creation (BRD lists "Registration/verification documents" as part of Option A) — page has a note pointing users to the org detail drawer's existing document upload after creation, since `SuperAdmin_CreateMemberWithOrg` doesn't touch `OrgDocuments` (Phase 1 scope was User+Org+OrgMembers only). Flag if inline upload during creation turns out to be required rather than deferred.

**Super Admin proactive Member + Organisation onboarding — Phase 1: backend core (2026-08-18)**
- Context: full BRD received for a new "Create Member" flow on the Super Admin Members page — proactively create a User + Organisation + role association before the person has ever logged in, then share a secure org profile link. This does NOT trace back to the approved v1.6 prototype (38 screens) — flagged to the user per the project's own rule that anything outside the prototype needs explicit discussion before build; the BRD itself is that discussion. Scoped into 4 phases; this entry covers Phase 1 only (backend core). Phases 2 (Super Admin wizard UI), 3 (rich public org profile — reviews/ratings/projects, current `Org_GetPublicPreview` is a thin card and does NOT cover this), and 4 (deep linking, needs a tool decision e.g. Branch.io since Firebase Dynamic Links is sunset) are tracked as roadmap items, not yet built.
- Findings that shaped scope: (1) the secure org link mechanism already exists — `IUrlTokenService` (AES-256-GCM, entity type `"ORG"`), same one `ShareController`/`PublicController` already use — reused directly, no new token system built. (2) `Auth_VerifyOTP` already looks up `Users` by Mobile then Email on every OTP verify — so a Super-Admin-pre-created `Users` row is automatically matched and reused on the person's real first login, with zero extra "claim" step; Phase 3 (BRD) needs no backend work beyond what's built here. (3) The existing `OrgInvitations` table/flow (v5.0) is a *different* feature — requires the invitee to already be logged in and explicitly Accept — not reused here since the BRD wants full pre-provisioning, not invite-then-accept.
- `NGOConnect_Complete_Setup_v5.0.sql`: new SP `SuperAdmin_CreateMemberWithOrg` — creates `Users` + `UserProfiles` + `Organisations` (if `OrgMode='NEW'`) + `OrgMembers` in one call. **First SP in this codebase to use an explicit `START TRANSACTION` / `EXIT HANDLER FOR SQLEXCEPTION` → `ROLLBACK`** — necessary because a partial failure must never orphan a User or Organisation row (explicit BRD requirement). Validates: at least one of Email/Mobile; duplicate Email/Mobile check; for `NEW` org mode, required OrgName/RegNumber/OrgTypeLkpId + duplicate RegNumber/OrgName check; for `EXISTING` mode, OrgId must exist; RoleCode must resolve against `MEMBER_ROLE` (FOUNDER/ADMIN/MODERATOR/MEMBER — no separate "Volunteer" org role exists in the lookup data, mapped conceptually to MEMBER). New User is `IsVerified=0` (flips to 1 on their real OTP login, same as self-registration). New Org (if created) is auto-`APPROVED`/`VERIFIED` — skips the normal self-service review queue since Super Admin is vetting it directly.
- `NGOConnect.Core/Models/SuperAdmin/SuperAdminModels.cs`: new `CreateMemberWithOrgRequest` (member fields, OrgMode NEW/EXISTING + full org fields, RoleCode).
- `NGOConnect.Core/Interfaces/ISuperAdminDal.cs` + `NGOConnect.Infrastructure/DAL/SuperAdminDal.cs`: new `CreateMemberWithOrgAsync` — calls the SP, then attaches the encrypted org share link (`_tokens.Encrypt("ORG", orgId)`, same as `CertificateDal.AttachVerifyLink` pattern) to the response. Added `IUrlTokenService` to `SuperAdminDal`'s constructor (already registered as a singleton — no DI registration change needed).
- `NGOConnect.API/Controllers/SuperAdminController.cs`: new `POST /api/v1/superadmin/members`.
- `Documents/patch_superadmin_member_onboarding.sql` (new) — no table/column changes, SP-only patch. Run on local → Railway staging → Railway production.
- `validate_sp_params.py` — all phases passed.
- Not build-verified this session (no local .NET SDK in the sandbox) — verify build before deploying.
- **Explicit design decision needing user confirmation before Phase 2 UI work**: duplicate Email/Mobile currently hard-blocks creation (BRD Section 13 lists this as an edge case to "handle", read here as reject-with-clear-message — if the intent was instead "associate with the existing user" that's a different SP branch and should be confirmed before the wizard UI is built against this contract).
- **API Documentation**: add `POST /api/v1/superadmin/members` + `SuperAdmin_CreateMemberWithOrg` at the next "update documents" pass. No existing SP/endpoint modified.
...
- `NGOConnect.Infrastructure/NGOConnect.Infrastructure.csproj`: added `PackageReference Microsoft.Extensions.Caching.Memory 8.0.1` — build initially failed with CS0246 (`IMemoryCache`/`Caching` not found) because this project had never referenced that package before. Restore NuGet packages in Visual Studio after pulling this change.
...
### [2026-08-26] Bug Fix: SuperAdmin_Org_SetNonRegistered allowed "Registered" with blank RegNumber
- **Root cause**: the SP only ever toggled `Organisations.IsNonRegistered` — it never checked `RegNumber` before allowing a flip to Registered (`IsNonRegistered = 0`), so Super Admin could mark an org Registered from the "Change Registration Status" panel in OrgDrawer even with no Registration Number on file.
- **DB** (`NGOConnect_Complete_Setup_v5.0.sql`): `SuperAdmin_Org_SetNonRegistered` — reads `RegNumber` alongside `StatusLkpId`; new guard blocks `IsNonRegistered = 0` when `RegNumber IS NULL OR TRIM(RegNumber) = ''`, returning `'Cannot mark as registered — a Registration Number is required first. Add it via Edit Organisation, then try again.'`. Patch: `patch_fix_setnonregistered_regnumber_guard.sql`.
- **Website**: no change needed — `OrgDrawer.jsx`'s `handleSaveRegStatus` already surfaces `res.message` via `alert()` on failure, so the new guard's error message displays as-is.
- **Verification**: `validate_sp_params.py` — all phases passed. No C# changes (message passthrough only).
