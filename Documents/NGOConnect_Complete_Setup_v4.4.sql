...
BEGIN
    ...
    SELECT
        ...
        p.CreatedAt,
        -- Haversine distance (km) — NULL when no user coords provided
        CASE
...
-- (Source: NGOConnect_Patch_OrgGetProfile_MemberStatus.sql)
...
BEGIN
    SELECT
        ...
        -- Impact stats (NOTE: sub-queries below reference non-existent columns;
        -- they will evaluate to NULL/0 until corrected in a future patch)
        IFNULL((SELECT SUM(TIMESTAMPDIFF(MINUTE, ps.StartTime, ps.EndTime)) / 60.0
        ...
        IFNULL((SELECT COUNT(DISTINCT p.OrgId)
                FROM ProjectAttendance pa JOIN Projects p ON pa.ProjectId = p.ProjectId
                WHERE pa.UserId = p_UserId AND pa.AttendanceStatus = 'ATTENDED'), 0) AS OrgCount,
        ROUND(IFNULL((SELECT attended / total * 100
                      FROM (SELECT SUM(CASE WHEN AttendanceStatus IN ('ATTENDED','EXCUSED') THEN 1 ELSE 0 END) AS attended,
                                   COUNT(*) AS total FROM ProjectAttendance WHERE UserId = p_UserId) r
                      WHERE total > 0), 100), 2) AS ReliabilityPct,
        IFNULL((SELECT COUNT(*) FROM ProjectAttendance WHERE UserId = p_UserId AND AttendanceStatus = 'NO_SHOW'), 0) AS NoShowCount,
        IFNULL((SELECT COUNT(*) FROM ProjectAttendance WHERE UserId = p_UserId AND AttendanceStatus = 'EXCUSED'), 0) AS ExcusedCount,
...
-- ── 3.24 Post_GetFeed ───────────────────────────────────────────────────────
-- Full rebuild: adds MediaUrls, MediaTypes, TimeAgo, IsPinned, PostTypeLkpCode
...
-- NGOConnect v4.4 — 50 Tables, 114 Stored Procedures
-- ============================================================
