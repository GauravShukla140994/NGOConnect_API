-- ============================================================
-- NGO Connect — Patch v4.5 → v4.6
-- Date   : 2026-07-11
-- Author : Claude (Software Architect)
--
-- Scope
-- ──────────────────────────────────────────────────────────
-- 1. Users table  : ADD COLUMN ProfileVerificationLkpId
-- 2. LookupTypes  : NEW type PROFILE_VERIFICATION_STATUS
--                   (PENDING / VERIFIED / NEEDS_UPDATE)
-- 3. SPs updated  : SuperAdmin_Org_GetDetail (+ MemberCount)
--                   SuperAdmin_Org_GetList   (+ OrgType)
-- 4. SPs new (11) : SuperAdmin_Org_GetStatusHistory
--                   SuperAdmin_User_GetList
--                   SuperAdmin_User_GetFullProfile
--                   SuperAdmin_User_GetDocuments
--                   SuperAdmin_UserDocument_Verify
--                   SuperAdmin_User_VerifyProfile
--                   SuperAdmin_User_RequestUpdate
--                   SuperAdmin_User_Suspend
--                   SuperAdmin_User_Reactivate
--                   SuperAdmin_Dashboard_GetKpis
--                   SuperAdmin_Org_GetRecent
--
-- C# changes (code-only, no DB impact)
-- ──────────────────────────────────────────────────────────
--   SuperAdminDal.cs  : Get<bool>() → Get<bool?>()
--                       Get<int>()  → Get<int?>()   (CS0019 fix)
--   NGOConnect.API.csproj : BCrypt.Net-Next 4.0.3 → 4.2.0  (NU1605 fix)
--
-- Prerequisites
-- ──────────────────────────────────────────────────────────
--   • NGOConnect_Patch_v4.5_Complete.sql already applied
--     (SuperAdminUsers table, OrgStatusHistory table, 18 original
--      SuperAdmin_* SPs must exist before this patch runs)
--
-- Safe to re-run: ALTER TABLE / INSERT guarded, SPs dropped first.
-- ============================================================

-- ─────────────────────────────────────────────────────────────
-- 1. Users table — add ProfileVerificationLkpId
-- ─────────────────────────────────────────────────────────────
...
SET @col_exists := (
    ...
    WHERE TABLE_SCHEMA = DATABASE()
      ...
      AND COLUMN_NAME  = 'ProfileVerificationLkpId'
);
...
SET @fk_exists := (
    ...
    WHERE TABLE_SCHEMA     = DATABASE()
      AND TABLE_NAME       = 'Users'
      AND CONSTRAINT_NAME  = 'fk_users_profileverification'
);
...
-- ─────────────────────────────────────────────────────────────
-- 2. Seed PROFILE_VERIFICATION_STATUS lookup
-- ─────────────────────────────────────────────────────────────

INSERT INTO LookupTypes (TypeCode, TypeName, Description, IsSystemType, CreatedBy)
SELECT 'PROFILE_VERIFICATION_STATUS',
       'Profile Verification Status',
       'Super Admin document/profile verification state for a member',
       1, 1
WHERE NOT EXISTS (
    SELECT 1 FROM LookupTypes WHERE TypeCode = 'PROFILE_VERIFICATION_STATUS'
);
...
WHERE lt.TypeCode = 'PROFILE_VERIFICATION_STATUS'
  AND NOT EXISTS (
    ...
    WHERE lv.LookupTypeId = lt.LookupTypeId AND lv.ValueCode = v.ValueCode
);

-- ─────────────────────────────────────────────────────────────
-- 3. Drop SPs before re-create
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 4a. SuperAdmin_Org_GetDetail — re-created, adds MemberCount
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 4b. SuperAdmin_Org_GetList — re-created, adds OrgType
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 5. NEW — SuperAdmin_Org_GetStatusHistory
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 7. NEW — SuperAdmin_User_GetFullProfile
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 8. NEW — SuperAdmin_User_GetDocuments
-- ─────────────────────────────────────────────────────────────
...
BEGIN
    SELECT ud.UserDocumentId, ud.DocumentTypeLkpId, dt.ValueName AS DocumentType,
           ud.FileUrl, ud.FileName, ud.IsVerified, ud.VerifiedAt, ud.VerifiedBy,
           ud.CreatedAt
...
-- ─────────────────────────────────────────────────────────────
-- 9. NEW — SuperAdmin_UserDocument_Verify
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 10. NEW — SuperAdmin_User_VerifyProfile
-- ─────────────────────────────────────────────────────────────
...
BEGIN
    ...
    ELSE
        SELECT LookupValueId INTO v_VerifiedId
        ...
        VALUES (p_UserId, 'PROFILE_VERIFIED',
...
-- ─────────────────────────────────────────────────────────────
-- 11. NEW — SuperAdmin_User_RequestUpdate
-- ─────────────────────────────────────────────────────────────
...
BEGIN
    ...
    ELSE
        SELECT LookupValueId INTO v_NeedsUpdateId
        FROM LookupValues lv
        JOIN LookupTypes lt ON lv.LookupTypeId = lt.LookupTypeId
...
-- ─────────────────────────────────────────────────────────────
-- 12. NEW — SuperAdmin_User_Suspend
-- ─────────────────────────────────────────────────────────────
...
BEGIN
    ...
    ELSE
        ...
        -- Revoke all active refresh tokens so the suspended member cannot silently
        -- keep a session alive or refresh to a new access token.
        -- NOTE: Does NOT block a brand-new login attempt — AuthDal does not check
        -- Users.IsActive. Closing that gap requires an explicit sign-off to edit AuthDal.
...
-- ─────────────────────────────────────────────────────────────
-- 13. NEW — SuperAdmin_User_Reactivate
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 14. NEW — SuperAdmin_Dashboard_GetKpis
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 15. NEW — SuperAdmin_Org_GetRecent
-- ─────────────────────────────────────────────────────────────
...
-- ============================================================
-- PATCH v4.6 — Addendum (2026-07-12)
-- Fix: SuperAdmin_User_GetList — include new users (no org)
--      + filter to APPROVED-only memberships
-- ============================================================
...
CREATE DEFINER=`root`@`%` PROCEDURE `SuperAdmin_User_GetList`(
    IN p_OrgIds     TEXT,
    IN p_Search     VARCHAR(150),
    IN p_PageNumber INT,
    IN p_PageSize   INT
...
BEGIN
    ...
    SELECT
        ...
        CONCAT(up.FirstName, ' ', up.LastName) AS FullName,
        u.Email, u.Mobile, up.ProfilePhoto,
        -- Show org names for APPROVED memberships only
        ...
        IF(u.IsActive = 1, 'ACTIVE', 'SUSPENDED') AS AccountStatus,
        COALESCE(pv.ValueCode, 'PENDING') AS ProfileVerificationStatus,
        -- New users (no org) ordered by account creation; approved members by first join
        COALESCE(
    ...
    FROM Users u
    JOIN  UserProfiles up ON up.UserId = u.UserId AND up.IsDeleted = 0
    -- LEFT JOIN: users with no membership still appear
    LEFT JOIN OrgMembers om ON om.UserId = u.UserId AND om.IsDeleted = 0
        AND (p_OrgIds IS NULL OR p_OrgIds = '' OR FIND_IN_SET(om.OrgId, p_OrgIds) > 0)
    ...
    HAVING
        -- Group 1: registered but not connected to any org yet (no org filter only)
        (COUNT(om.OrgMemberId) = 0 AND (p_OrgIds IS NULL OR p_OrgIds = ''))
        OR
        -- Group 2: at least one APPROVED membership in the filtered orgs (or any org)
        SUM(CASE WHEN sv.ValueCode = 'APPROVED' THEN 1 ELSE 0 END) > 0
    ORDER BY JoinedAt DESC
    LIMIT p_PageSize OFFSET v_Offset;

    -- TotalCount mirrors HAVING exactly
    SELECT COUNT(*) AS TotalCount FROM (
        SELECT u.UserId
        FROM Users u
        JOIN  UserProfiles up ON up.UserId = u.UserId AND up.IsDeleted = 0
...
-- ============================================================
-- END OF PATCH v4.6
-- Total new SPs: 11   |   Updated SPs: 3   |   New columns: 1
-- New LookupTypes: 1  |   New LookupValues: 3
-- ============================================================
